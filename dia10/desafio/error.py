class DimensionError(Exception):
    ...
    def __str__(self):
        # Sobrecarga del método __str__ para generar mensajes de error
        if self.dimension is not None and self.maximo is not None:
            # Error cuando están todos los datos.
            return f"{self.mensaje} La dimensión ingresada es: {self.dimension} y el máximo es: {self.maximo}."
        else:
            return super().__str__()  # En caso de que no se cumplan esas condiciones y se llame al error, llamado al método padre con super, solo retorna el "mensaje"
