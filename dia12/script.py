...
try:
    with open("dia12/usuarios.txt", "r") as usuarios: #abrir archivo en modo lectura con un alias
        for linea in usuarios.readlines(): #recorrer por línea el archivo con todas sus líneas
            try: #control de excepciones
                ...
                with open(f"dia12/logs/error.log", "a+",encoding="utf-8") as log: #y se escribe en error.log cuál fue el error
                    log.write(f"Error al procesar la línea: {linea}\tExcepción: {e}\n")
    ...
    with open(f"dia12/logs/error.log", "a+",encoding="utf-8") as log:
        log.write(f"Error al abrir el archivo.\n\tExcepción: {error}\n")